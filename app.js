...
function paintCompaniesOptions(symbol, snap, placeholder = "—") {
  ...
  rows.forEach((row) => {
    ...
    set("last", last, last
      ? `Actual gap moves at the last three earnings: ${last}`
...
function renderCompanies(calls) {
  ...
  const optionCells = showOpts
    ? `<div class="num" data-opt="move">…</div>
        ...
        <div class="num last-cell" data-opt="last" title="Actual gap moves at the last three earnings">…</div>
